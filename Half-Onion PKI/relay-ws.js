...
wsServer.on('request', function(request) {
  ...
  connection.on('message', async function(message) {
    ...
    try {
      ...
      if (parsed.type === "key-signed" || parsed.type === "talk") {
        if (!verifyFromCA(parsed.jwt, parsed)) {
          throw new Error("Message is not trusted.");
        }
        console.log("✅ Verified trusted CA message");
        // Relay or process the message now
      }
      // {test 2}
    } catch (e) {
    ...
    // {test 1}
    parsed.peerlength = allConnections[id].length;
    for (var i=0; i<allConnections[id].length; i++) {
      ...
      if (message.type === 'utf8') {
        c.sendUTF(JSON.stringify(parsed));
      } else if (message.type === 'binary') {
        // {test 8}
        c.sendBytes(parsed.binaryData);
      }
    }
  });
